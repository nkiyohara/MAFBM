...
def gammaincc_ez_fractions(a, z, num_fractions=5):
    ...
    a_m = [1] + [
        i // 2 - a if i % 2 == 0 else i // 2 for i in range(2, num_fractions + 1)
    ]
    g = 0.0
...
def gamma_by_r(num_k, r, offset=0.0):
    n = (num_k + 1) / 2 + offset
...
def gamma_by_gamma_max(num_k, gamma_max, offset=0.0):
...
def omega_riemann(gamma, hurst):
    ...
    omegas = jnp.where(
        ...
        / sp.gamma(1 - alpha)
        * (
            (d2ma[:-1] / (2 - alpha) - g[:-2] * d1ma[:-1] / (1 - alpha)) / d1[:-1]
            + (g[2:] * d1ma[1:] / (1 - alpha) - d2ma[1:] / (2 - alpha)) / d1[1:]
        ),
        -1
        / sp.gamma(alpha)
        / sp.gamma(2 - alpha)
        / (2 - alpha)
        * (d2ma[:-1] / d1[:-1] - d2ma[1:] / d1[1:]),
    )
    return omegas
...
def omega_optimized_1(gamma, hurst, time_horizon, return_cost=False):
    ...
    A = (
        2 * time_horizon
        + (jnp.exp(-gamma_i * time_horizon) - 1) / gamma_i
        + (jnp.exp(-gamma_j * time_horizon) - 1) / gamma_j
    ) / (gamma_i + gamma_j)
    b = (
        2 * time_horizon / gamma ** (hurst + 0.5)
        - time_horizon ** (hurst + 0.5) / (gamma * sp.gamma(hurst + 1.5))
        + (
            jnp.exp(-gamma * time_horizon)
            - gammaincc_ez(hurst + 0.5, gamma * time_horizon)
        )
        / gamma ** (hurst + 1.5)
    )
...
def omega_optimized_2(gamma, hurst, time_horizon, return_cost=False):
    ...
    A = (
        time_horizon
        + (jnp.exp(-(gamma_i + gamma_j) * time_horizon) - 1) / (gamma_i + gamma_j)
    ) / (gamma_i + gamma_j)
    b = time_horizon / gamma ** (hurst + 0.5) * sp.gammainc(
        hurst + 0.5, gamma * time_horizon
    ) - (hurst + 0.5) / gamma ** (hurst + 1.5) * sp.gammainc(
...
def solve_vector(params, model, omega, x0, y0, t0, num_steps, dt, key, args):
    ...
    def step(z, i):
        ...
        dx = model.b(params, t, x, args) * dt + model.s(params, t, x, args) * (
            omega[None, :] * dy
        ).sum(axis=-1)
    ...
    log_path = 0.5 * (us**2).sum(axis=0) * dt
...
class CustomPath(diffrax.AbstractPath):
    ...
    def __init__(self, t0, t1, tol, num_latents, num_k, key):
        ...
        self.t0 = t0
        self.t1 = t1
        self.brownian_path = diffrax.VirtualBrownianTree(
            t0=t0, t1=t1, tol=tol, shape=(num_latents,), key=key
        )
...
def solve_diffrax(
    params, model, omega, x0, y0, ts, dt, key, solver=diffrax.Euler(), args=None
):
    def drift(t, state, args):
        x, y, _ = state
        u = model.u(params, t, x, y, args)
        dy = -model.gamma[:, None] * y + u[None, :]
        dx = model.b(params, t, x, args) + model.s(params, t, x, args) * (
            omega[:, None] * dy
        ).sum(axis=0)
        return (dx, dy, 0.5 * u**2)

    def diffusion(t, state, args):
        x, _, _ = state
        return (
            model.s(params, t, x, args) * omega.sum(),
            jnp.ones((model.num_k, model.num_latents)),
            jnp.zeros(model.num_latents),
        )

    state_init = (x0, y0.T, jnp.zeros(model.num_latents))
    brownian_motion = CustomPath(
        ts[0], ts[-1], dt / 10, model.num_latents, model.num_k, key
    )
